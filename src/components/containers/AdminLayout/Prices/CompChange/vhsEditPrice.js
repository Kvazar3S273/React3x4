...
import { GetVhses, UpdateVhsTable } from "../../../../../constants/actions/compActions/vhs";

const VhsEditPrice = () => {
  const input = useRef();

  const initialVhsState = {
    id: null,
    price: null,
  };
  ...
  const [editVhs, setEditVhs] = useState(initialVhsState);
    ...
    <div className="row">
      ...
      <div className="col-md-10">
        <table className="table table-striped text-center">
          ...
          <tbody>
            {listvhses.map((row, index) => (
              <TableRow key={row.id} row={row} index={listvhses[index]} />
            ))}
          </tbody>
        </table>
      </div>
  ...
  const RowHandleClick = (e, row) => {
    ...
    console.log("Row", row);
    const item = listvhses.find((row) => row.id == e.currentTarget.id);
  ...
  const handleInputChange = (dataType, values, index) => {
    setEditVhs({ ...editVhs, [dataType]: values });
  };
  ...
  const updateVhsItem = () => {
    const idItem = editVhs.id;

    const upd = {
      price: editVhs.price,
    };

    dispatch(UpdateVhsTable(idItem, upd))
      .then((res) => {
  ...
  return (
    <div className="row mt-3 mb-3" style={{overflowX:"auto"}}>
      <div className="col py-3" style={{ backgroundColor: "#e0e3e5" }}>
        ...
        <Table listvhses={listvhses} />
      </div>
    </div>
  );
};
export default VhsEditPrice;
